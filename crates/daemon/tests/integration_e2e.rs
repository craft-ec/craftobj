...
/// repeatedly to generate demand. B's DemandTracker records
/// inbound fetch requests → HealthScan skips degradation despite over-replication.
...
async fn test_degradation_skipped_with_demand() -> Result<(), String> {
    init_test_tracing();
    info!("=== Running test_degradation_skipped_with_demand ===");
