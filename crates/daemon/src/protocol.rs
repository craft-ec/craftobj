    ...
    async fn handle_query_result(
        ...
        match (pending, result) {
            ...
            (PendingQuery::ManifestLookup { content_id }, QueryResult::GetRecord(Ok(GetRecordOk::FoundRecord(peer_record)))) => {
                ...
                match craftobj_routing::parse_manifest_record(&peer_record.record.value) {
                    Ok(manifest) => {
                        let event = CraftObjEvent::ManifestRetrieved {
                            content_id,
                            manifest,
                        };
            ...
            (PendingQuery::AccessListLookup { content_id }, QueryResult::GetRecord(Ok(GetRecordOk::FoundRecord(peer_record)))) => {
                ...
                match craftobj_routing::parse_access_list_record(&peer_record.record.value) {
                    Ok(access_list) => {
                        let event = CraftObjEvent::AccessListRetrieved {
                            content_id,
                            access_list,
                        };
                        if let Err(e) = self.event_tx.send(event) {
